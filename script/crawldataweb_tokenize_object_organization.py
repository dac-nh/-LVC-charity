...
def extract_attribute(content, attribute, attribute_data, attribute_word, n_attribute, percent, general_attribute):
  ...
  word_frequency_df = word_frequency_df[word_frequency_df['stop_word'] == 'non_stop_word'].drop(['stop_word'], axis = 1).sort_values('frequency', ascending = False)[:round(len(word_frequency_df)*percent)]
...
# content = df['Content'][232]

# content = word_tokenize(pattern.sub('', content))
...
# word_frequency_df['stop_word'] = word_array
# word_frequency_df = word_frequency_df[word_frequency_df['stop_word'] == 'non_stop_word'].drop(['stop_word'], axis = 1).sort_values('frequency', ascending = False)[:round(len(word_frequency_df)*1)]
# word_frequency_df = word_frequency_df['word'].tolist()
# word_frequency_df
...
objects = []
for i in range(n):
  objects.append(extract_attribute(df['Content'][i], 'object', object_words, 'object_word', n_object, 0.2, 'Người dân'))
...
for i in range(n_kindmate):
  organizations.append(extract_attribute(df['Content'][i], 'org', org_words, 'org_word', n_org, 1, 'Vui lòng đọc bài đầy đủ ở link để biết thông tin chi tiết!'))
